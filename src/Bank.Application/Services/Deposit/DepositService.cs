...
public class DepositService : IDepositService
{
    ...
    public async Task<DepositCertificateDto> GenerateCertificateAsync(Guid depositId, Guid generatedByUserId)
    {
        ...
        var certificateContent = $"{certificate.CertificateNumber}{certificate.FixedDepositId}{certificate.IssueDate:yyyyMMddHHmmss}";
        using var sha256 = System.Security.Cryptography.SHA256.Create();
        var hashBytes = sha256.ComputeHash(System.Text.Encoding.UTF8.GetBytes(certificateContent));
...
";
    ...
    private async Task<FixedDepositDto> MapToFixedDepositDtoAsync(FixedDeposit deposit)
    {
        ...
        return await DepositMappingHelper.MapToFixedDepositDtoAsync(deposit);
    }

    private static DepositCertificateDto MapToDepositCertificateDto(DepositCertificate certificate)
        => DepositMappingHelper.MapToDepositCertificateDto(certificate);

    private static MaturityNoticeDto MapToMaturityNoticeDto(MaturityNotice notice)
        => DepositMappingHelper.MapToMaturityNoticeDto(notice);
